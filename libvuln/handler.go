...
func (h *HTTP) VulnerabilityReport(w http.ResponseWriter, r *http.Request) {
	...
	zlog.Info(ctx).Msg("Testing request_id is logged - info level")
	zlog.Warn(ctx).Msg("Testing request_id is logged - warn level")
	zlog.Error(ctx).Msg("Testing request_id is logged - error level")
